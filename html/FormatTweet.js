...
function FormatImageGallery(n, users) {
	var rv = "";
	rv = rv + "\
	<p class=\"comparison-intro\">\"Let's have some fun with photos!\"</p>";
	for (var j = 0; j < n.length; j++) {
	if (n.length == 0) return "";
	rv = rv + "\
<div><h6 class=\"comparison-subtitle\">Photos posted by " + users[j] + "...</h6></div>\
...
function FormatTopics(categorylist, topicPercentage) {
	var rv = "\
		<div><h6 class=\"comparison-subtitle\" >They are talking about...</h6></div>\
		<p class=\"comparison-intro\">\"Want to know common topics? TwConnect analyzes tweets and hashtags to find them!\"</p>\
		<div class=\"word-cloud\">\
			<wordcloud></wordcloud>\
		</div>\
		<div><h6 class=\"comparison-subtitle\">Tweets in categories...</h6></div>\
		<p class=\"comparison-intro\">\"Based on keywords, TwConnect separates tweets into various categories.\"</p>\
...
function FormatTopicPanel(panelname, percentage, subtopics, subtopic_percentage, index) {
	var u1percentage = percentage["user1percentage"];
	var u2percentage = percentage["user2percentage"];
	if (u1percentage > 1) u1percentage = 1;
	if (u2percentage > 1) u2percentage = 1;
	var rv = "<div class=\"panel panel-default\">\
				<div class=\"panel-heading topic-panel-heading\" data-toggle=\"collapse\" data-parent=\"#topic-panel\" href=\"#topic"+index+"\">\
					<div class=\"mbar-left-blank\" style=\"width: " + (30 - u1percentage * 100 * .30).toString() + "%\"></div>\
					<div class=\"mbar-left\" style=\"width: " + (5 + u1percentage * 100 * .30).toString() + "%\">\
						<div class=\"topic-percentage\">" + Math.floor(u1percentage * 100) + "%&nbsp;&nbsp;</div>\
					</div>\
					<div class=\"panel-title topic-bar\">" + panelname + "</div>\
					<div class=\"mbar-right\" style=\"width: " + (5 + u2percentage * 100 * .30).toString() + "%\">\
						<div class=\"topic-percentage\">&nbsp;&nbsp;" + Math.floor(u2percentage * 100) + "%</div>\
					</div>\
					<div class=\"mbar-right-blank\" style=\"width: " + (30 - u2percentage * 100 * .30).toString() + "%\"></div>\
				</div>\
	...
	for (var i = 0; i < subtopic_percentage.length; i++) {
		if (parseInt(subtopic_percentage[i][1]) > parseInt(largest)) {
			largest = subtopic_percentage[i][1];
		}
		if (parseInt(subtopic_percentage[i][2]) > parseInt(largest)) {
			largest = subtopic_percentage[i][2];
		}
	}

	for (var i = 0; i < subtopic_percentage.length; i++) {
		var leftwidth = Math.floor(subtopic_percentage[i][1] / largest * 100);
		var rightwidth = Math.floor(subtopic_percentage[i][2] / largest * 100);
...
function FormatTopicKeyword(keyword, widthleft, widthright, panelname) {
	var rv = "<div class=\"subtopic\">\
					<div class=\"bar-left-outer\"><div class=\"bar-left\" style=\"width: " + widthleft + "%\"></div></div>\
					<div class=\"subtopic-bar\"   data-toggle=\"modal\" data-target=\"#myModal\" id=\"subtopic-refer\" topic=\"" + panelname + "\">" + keyword + "</div>\
					<div class=\"bar-right-outer\"><div class=\"bar-right\" style=\"width: " + widthright + "%\"></div></div>\
				</div>";
	return rv;
}
...
function FormatPeople(commonfollowers, user1, user2) {
	var rv = "\
		...
		<h6 class=\"comparison-subtitle\">They both follow...</h6>\
